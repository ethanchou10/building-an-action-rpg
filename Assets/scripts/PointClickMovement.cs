	...
	void Update() {
		...
		if (_targetPos != null) {
			if (_curSpeed > moveSpeed * .5f) { // lock rotation when stopping
				Vector3 adjustedPos = new Vector3(_targetPos.Value.x, transform.position.y, _targetPos.Value.z);
			...
			if (Vector3.Distance(_targetPos.Value, transform.position) < targetBuffer) {
